...
def show_all_users(message):
    ...
    if user_id in admin_id:
        ...
        response = "Only @mohit72jd Can Run This Command."
    bot.reply_to(message, response)

@bot.message_handler(commands=['owner'])
def show_owner(message):
    response = "👑 Bot Owner: @mohit72jd"  # Replace with the actual owner username
    bot.reply_to(message, response)
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        response = ("🚫 Unauthorized Access! 🚫\n\n"
                    ...
                    "👉 Contact an Admin or the Owner @mohit72jd for approval.\n"
                    "🌟 Become a proud supporter and purchase approval.\n"
                    "💬 Chat with an Owner @mohit72jd now and level up your capabilities!\n\n"
...
def clear_logs_handler(message):
    ...
    if user_id in admin_id:
        ...
        bot.reply_to(message, "Only @mohit72jd can run this command.")
